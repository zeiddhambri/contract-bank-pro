    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          ...
          return (
            <Card key={index} className="bg-black/40 border-slate-700/50 backdrop-blur-sm hover:bg-black/50 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/10 group">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    {isEditing ? (
                      <div className="flex items-center space-x-2">
                        <Input
                          value={tempValues[stat.id as keyof typeof tempValues]}
                          onChange={(e) => handleTempValueChange(stat.id, e.target.value)}
                          onKeyDown={(e) => handleKeyPress(e, stat.id)}
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleEditClick(stat.id)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {financialStats.map((stat, index) => {
          ...
          const isEditing = editingField === stat.id;
          return (
            <Card key={index} className="bg-gradient-to-r from-orange-600/20 to-red-600/20 border-orange-500/30 backdrop-blur-sm hover:from-orange-600/30 hover:to-red-600/30 transition-all duration-300 group">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                    ...
                    {isEditing ? (
                      <div className="flex items-center space-x-2">
                        <Input
                          ...
                          onChange={(e) => handleTempValueChange(stat.id, e.target.value)}
                          onKeyDown={(e) => handleKeyPress(e, stat.id)}
                          className="text-3xl font-bold bg-black/20 border-slate-600 text-white h-10 px-2"
                          autoFocus
                        />
                        <div className="flex space-x-1">
                          <Button
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => handleEditClick(stat.id)}
                          className="h-6 w-6 p-0 text-orange-200 hover:text-white hover:bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          <Edit className="h-3 w-3" />
                        </Button>
                      </div>
